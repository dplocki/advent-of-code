...
def parse(task_input: list[str]) -> list[tuple[bool, int, int, int, int, int, int]]:
    pattern = re.compile(r'(on|off) x=(-?\d+)\.\.(-?\d+),y=(-?\d+)\.\.(-?\d+),z=(-?\d+)\.\.(-?\d+)')

    for line in task_input:
        groups = pattern.match(line)
        yield groups[1] == 'on', int(groups[2]), int(groups[3]), int(groups[4]), int(groups[5]), int(groups[6]), int(groups[7])


def solution_for_first_part(task_input: list[str]) -> int:
    reactors = set()

    for state, xs, xe, ys, ye, zs, ze in parse(task_input):
        xs = max(-50, xs)
        xe = min(50, xe)
        ys = max(-50, ys)
        ye = min(50, ye)
...
def solution_for_second_part(task_input: list[str]) -> int:
    instructions = list(reversed(list(parse(task_input))))

    all_x = set()
    all_y = set()
    all_z = set()
    for _, xs, xe, ys, ye, zs, ze in instructions:
        all_x.add(xs)
        all_x.add(xe + 1)
        all_y.add(ys)
        all_y.add(ye + 1)
    ...
    for x1, x2 in zip(all_x, all_x[1:]):
        matching_for_x = [(state, ys, ye, zs, ze) for state, xs, xe, ys, ye, zs, ze in instructions if xs <= x1 <= xe]
        for y1, y2 in zip(all_y, all_y[1:]):
            matching_for_y = [(state, zs, ze) for state, ys, ye, zs, ze in matching_for_x if ys <= y1 <= ye]
            for z1, z2 in zip(all_z, all_z[1:]):
                if next((state for state, zs, ze in matching_for_y if zs <= z1 <= ze), False):
...
assert solution_for_second_part(example_input) == 2758514936282235
